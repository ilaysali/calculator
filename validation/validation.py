...
def validate_input ( current:tuple, previous: tuple = None) -> None:
    current_token = current[-1] if is_number(current[0]) else get_operator(current[0])
    previous_token = (previous[-1] if is_number(previous[0]) else get_operator(previous[0])) if previous else None
...
def validate_numbers_LParentheses_Lunary (previous_token: str, current_token: str) -> None:
    if previous_token and previous_token in ["Number", ')', '!', '#']:
        raise InvalidInputException(f"{current_token} cant come after {previous_token}")
    if current_token != '(' and current_token == previous_token :
        raise InvalidInputException(f"{current_token} cant come after itself")

def validate_operators_RParentheses_Runary (previous_token: str, current_token: str) -> None:
    if not previous_token:
        raise InvalidInputException(f"{current_token} cant be at the start of input")
